...
### 新增
- **监控项导入导出**：支持 Excel 格式批量导入/导出监控项，导入时可选择告警通道替换策略
- **域名证书星标关注**：运维/管理员可为域名设置星标，支持"只看关注"筛选，星标项置顶排序
- **告警静默永久生效**：无起止日期的静默规则默认永久生效，支持跨夜时段（如 22:00~06:00）
- **告警静默规则说明**：静默时段标签旁增加提示图标，悬浮展示一次性/周期性/跨夜场景的规则说明

### 修复
- **监控任务告警恢复通知**：修复恢复通知不发送的问题——静默时未记录失败计数导致恢复检测失败；周期性恢复需连续成功多次但只触发一次检测，现每次成功都通知 AlertService 判断
- **告警静默跨夜+周期性**：修复 weekly/monthly 的 days/dates 从前端 config 顶层读取（之前错误地从 window 对象中查找，导致星期/日期过滤永远不生效）
- **监控任务编辑后添加500**：编辑保存后表单残留旧 id，创建新任务时提交多余字段导致500
- **监控项导入告警渠道为空**：导入弹窗打开时未加载告警渠道列表，选择"手动替换"后下拉为空
- **非HTTP监控项告警通道状态**：告警通道下拉选项增加启用/禁用状态标签
- **Excel 导出 undefined**：前端拦截器已解包 response.data，导出时又取 res.data 导致 Blob 变为 undefined
- **域名证书关注列对 VIEWER 可见**：el-table-column 用 v-permission 无效（display:none 不影响列渲染），改用 v-if

### 安全
- **ReminderController 鉴权**：所有写操作增加 @PreAuthorize，VIEWER 不再能通过 API 创建/删除提醒
- **admin 用户保护**：admin 用户只能由本人修改个人信息，不能被其他管理员踢出会话
- **运维角色权限扩展**：代理设置（全部接口）、数据备份（列表/创建/恢复/上传恢复/数据清理）放开给 OPERATOR

### 优化
- **VIEWER 角色菜单**：隐藏周期提醒菜单和仪表盘周期提醒按钮；域名证书/监控任务/告警记录页不再调用无权限的 alert/channel/list 接口
- **告警静默默认永久**：无起止日期时自动启用，不再报500

### 文档
- 更新 Manual.vue（告警静默、导入导出章节）
- 更新 changelog.md、devlog.md、README.md
