...
### 修复
- **周期提醒重复通知**：过期任务同时发送提前提醒和到期提醒，现过期时跳过提前提醒只发到期提醒
- **恢复通知不发送**：连续失败触发告警后计数器归零导致恢复检测失败；恢复通知功能错误依赖限频开关（默认关闭），现已独立
- **告警模板恢复通知可见性**：恢复通知和连续正常次数配置项始终展示，无需先启用限频

### 新增
- **数据清理**：数据备份页面新增「数据清理」按钮（仅管理员），支持按天数独立清理执行日志、告警记录、巡检记录

### 文档
- 更新 Manual.vue（告警限频章节、数据备份章节）
- 更新 changelog.md、devlog.md
