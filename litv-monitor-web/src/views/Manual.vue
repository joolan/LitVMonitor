      ...
      <el-collapse v-model="activeSections">
        ...
        <el-collapse-item name="quickstart">
          ...
          <div class="section-content">
            ...
            <p>支持<b>Excel批量导入/导出</b>：点击"导入Excel"上传包含监控项配置的表格，可选择是否替换告警通道；点击"导出Excel"下载当前所有监控项。</p>
  ...
  request.params['t'] = util.timestamp();
        ...
        <el-collapse-item name="alert">
          ...
          <div class="section-content">
            ...
            <h4>告警静默（维护窗口）</h4>
            <p>在维护期间暂停告警通知，避免产生大量无意义告警。静默期间告警仍会记录，仅不发送通知，且不消耗限频配额。</p>
            <ul>
              <li><b>一次性静默</b>：指定起止日期，期间每天的静默时段都生效（如 9:00~18:00 表示生效日期内每天 9点到18点静默）</li>
              <li><b>永久静默</b>：不设起止日期，配合时间段实现长期静默规则</li>
              <li><b>周期性静默</b>：按每日/每周/每月重复，匹配日期 + 时段
                <ul>
                  <li>每天：可配置多个时间段，如 02:00~06:00</li>
                  <li>每周：指定星期几 + 时间段（如每周一 10:00~18:00）</li>
                  <li>每月：指定几号 + 时间段（如每月15号 09:00~18:00）</li>
                </ul>
              </li>
              <li><b>跨夜时段</b>：开始时间大于结束时间（如 22:00~06:00）表示跨夜静默。注意：周期性规则中跨夜时段仅对匹配日期生效，如"每周一 22:00~08:00"，周二 0:00-8:00 不会静默</li>
        ...
        <el-collapse-item name="domain">
          ...
          <div class="section-content">
            ...
            <h4>星标关注（运维/管理员）</h4>
            <ul>
              <li>可为重要域名设置星标关注，星标项在列表中置顶显示</li>
              <li>支持"只看关注"筛选，快速查看重点关注的域名</li>
              <li>点击星标图标即可切换关注状态</li>
            </ul>
          </div>
        </el-collapse-item>
        ...
        <el-collapse-item name="system">
          ...
          <div class="section-content">
            ...
            <ul>
              <li><b>admin 账号保护</b>：admin 用户只能由本人修改个人信息（昵称、邮箱、密码），其他管理员不能修改；admin 不能被踢出会话</li>
            </ul>

            <h4>数据备份（ADMIN/OPERATOR）</h4>
            <ul>
              ...
              <li><b>数据清理</b>：按天数清理历史数据，支持独立配置执行日志、告警记录、巡检记录的保留天数（默认 30 天）</li>
              <li><b>下载/删除备份</b>：仅管理员可操作</li>
        ...
        <el-collapse-item name="security">
          ...
          <div class="section-content">
            ...
            <h4>用户管理增强</h4>
            <ul>
              <li><b>Admin 账号保护</b>：admin 账号不能被删除、禁用或修改角色；admin 用户只能由本人修改个人信息，其他管理员不能修改；admin 不能被踢出会话</li>
...
const roles = [
  { role: 'ADMIN', desc: '管理员，所有操作权限，包括用户管理、安全设置、代理、备份下载/删除' },
  { role: 'OPERATOR', desc: '运维，监控/任务/变量/告警/代理设置/数据备份的增删改查，不可管理用户和安全设置' },
